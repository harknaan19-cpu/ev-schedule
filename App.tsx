...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const color = isDarkMode ? '#0f1117' : '#f0f4f8';
      ...
      <header className="sticky top-0 z-40 w-full glass-header a-fade-in" style={{ paddingTop: 'env(safe-area-inset-top)' }}>
        <div className="max-w-5xl mx-auto px-4 md:px-6 h-14 md:h-16 flex items-center justify-between">
          <div className="flex items-center gap-2 md:gap-3">
            <svg className="w-7 h-7 md:w-8 md:h-8 flex-shrink-0" viewBox="0 0 64 64" fill="none">
              <circle cx="32" cy="32" r="29" stroke="currentColor" strokeWidth="4" fill="none" className="text-slate-600 dark:text-slate-300"/>
              <polygon points="30,8 46,44 14,44" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinejoin="round" className="text-slate-600 dark:text-slate-300"/>
              <line x1="30" y1="8" x2="38" y2="26" stroke="currentColor" strokeWidth="2" strokeLinecap="round" className="text-slate-600 dark:text-slate-300"/>
              <polygon points="20,24 34,44 6,44" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinejoin="round" className="text-slate-600 dark:text-slate-300"/>
              <line x1="20" y1="24" x2="14" y2="36" stroke="currentColor" strokeWidth="2" strokeLinecap="round" className="text-slate-600 dark:text-slate-300"/>
              <rect x="38" y="34" width="16" height="13" rx="3" fill="currentColor" className="text-slate-600 dark:text-slate-300"/>
              <text x="46" y="44.5" textAnchor="middle" fontFamily="Arial,Helvetica,sans-serif" fontWeight="800" fontSize="10" fill="currentColor" className="text-white dark:text-slate-800">19</text>
            </svg>
            <h1 className="text-base md:text-xl font-extrabold tracking-tight truncate">
              הר כנען <span className="text-[#9B1B5E] dark:text-[#b84d7e]">19</span>
            </h1>
          </div>
        ...
        <div className="glass-card p-1.5 rounded-full mb-6 md:mb-8 a-fade-up a-d1">
          <div className="flex gap-1.5">
            ...
            <button onClick={() => { setActiveTab('club'); setShowHistory(false); setShowStats(false); }}
              ...
              <svg className={`w-4 h-4 ${activeTab === 'club' ? 'text-[#9B1B5E]' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" /></svg>
              מועדון דיירים
            </button>
          </div>
        </div>
            ...
            <div className="flex items-center gap-2">
              <button onClick={() => { const next = !showHistory; setShowHistory(next); setShowStats(next); }}
                className={`text-[10px] md:text-xs font-bold px-3.5 py-1.5 rounded-full transition-all duration-300 spring flex items-center gap-1.5 ${
                  showHistory ? 'glass-tab-active text-[#9B1B5E] dark:text-[#b84d7e]' : 'glass-btn text-slate-400'
                }`}>
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <button onClick={() => setShowStats(!showStats)}
                  className={`mr-auto text-[10px] md:text-xs font-bold px-3.5 py-1.5 rounded-full transition-all duration-300 spring flex items-center gap-1.5 ${
                    showStats ? 'glass-tab-active text-[#9B1B5E] dark:text-[#b84d7e]' : 'glass-btn text-slate-400'
                  }`}>
              ...
              {showStats && (
                <div className="glass-card rounded-3xl p-4 md:p-6 a-scale-in">
                  <h3 className="text-sm font-bold mb-4 tracking-tight flex items-center gap-2">
                    <svg className="w-4 h-4 text-[#9B1B5E]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M16 8v8m-4-5v5m-4-2v2m-2 4h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
          ...
          {isLoading ? (
            <div className="flex flex-col items-center justify-center py-24 gap-5 a-fade-in">
              <div className="relative w-14 h-14 glass-card rounded-full flex items-center justify-center">
                ...
                <div className="absolute w-9 h-9 border-3 border-t-[#9B1B5E] rounded-full animate-spin"></div>
              </div>
