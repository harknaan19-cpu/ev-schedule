...
const ClubBookingForm: React.FC<ClubBookingFormProps> = ({ existingBookings, onSuccess, onCancel }) => {
  ...
  const selCls = "w-full px-4 py-3 glass-input rounded-full outline-none text-slate-700 dark:text-slate-200 font-medium appearance-none transition-all duration-200";
  const disCls = "w-full px-4 py-3 glass-input rounded-full outline-none text-slate-400 font-medium appearance-none cursor-not-allowed opacity-60";
  ...
  return (
    <div className="p-6 md:p-8">
      ...
      {dateBookings.length > 0 && !isFullyBooked && (
        <div className="mb-5 p-3 rounded-3xl text-xs md:text-sm font-semibold text-cyan-800 dark:text-cyan-300 a-fade-up a-d1 flex items-center gap-2 bg-cyan-50 dark:bg-cyan-500/10">
      ...
      {isFullyBooked && (
        <div className="mb-5 p-3 rounded-3xl text-red-500 text-sm font-semibold text-center a-scale-in bg-red-50 dark:bg-red-500/10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="a-fade-up a-d4">
            <label className={`flex items-center gap-3 px-4 py-3 rounded-full transition-all duration-200 ${
              isClubTaken ? 'glass-input cursor-not-allowed opacity-60' : 'glass-card-subtle cursor-pointer'
            }`}>
        ...
        <div className="flex gap-3 a-fade-up a-d6">
          ...
          <button type="submit" disabled={isFullyBooked}
            className={`flex-[2] font-bold py-3.5 rounded-full text-base flex items-center justify-center gap-2 group ${
              isFullyBooked ? 'glass-input text-slate-400 cursor-not-allowed opacity-60' : 'glass-btn-primary'
            }`}>
            <span>הזמן</span>
      ...
      {error && (
        <div className="mt-5 p-4 rounded-3xl text-red-500 text-sm font-semibold flex items-center gap-3 a-scale-in bg-red-50 dark:bg-red-500/10">
